...
import { MapPin, ChevronRight, Box, Footprints, Camera } from 'lucide-react';
  ...
  return (
    <AppShell accent="blue" title="Schadensverwaltung" subtitle={`Hallo ${profile?.full_name ?? ''}`}>
      <div className="space-y-4 px-4 py-4">
        ...
        <Link
          to="/erfasser/ar"
          className="flex w-full items-center gap-4 rounded-2xl bg-slate-800 px-4 py-5 text-white shadow-lg active:scale-[0.98] transition"
        >
          <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-white/20">
            <Camera className="h-7 w-7" />
          </div>
          <div className="text-left">
            <div className="text-lg font-semibold">AR-Ansicht</div>
            <div className="text-sm text-slate-300">Schäden &amp; Objekte im Kamerabild finden</div>
          </div>
        </Link>
